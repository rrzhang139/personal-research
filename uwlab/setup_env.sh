...
echo "[2/6] Installing system deps (libGLU for rendering)..."
apt-get install -y libglu1-mesa > /dev/null 2>&1

echo "[3/6] Installing Isaac Sim 5.1.0 (this takes ~10 min)..."
uv pip install "isaacsim[all,extscache]==5.1.0" --extra-index-url https://pypi.nvidia.com

echo "[4/6] Installing PyTorch 2.7.0 with CUDA 12.8..."
uv pip install -U torch==2.7.0 torchvision==0.22.0 --index-url https://download.pytorch.org/whl/cu128

echo "[5/6] Cloning UWLab..."
git clone https://github.com/uw-lab/UWLab.git 2>/dev/null || echo "UWLab already cloned"
cd UWLab

echo "[6/7] Installing UWLab + rsl_rl..."
./uwlab.sh --install
