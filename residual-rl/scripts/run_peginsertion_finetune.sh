...
echo "Starting PegInsertionSide-v1 FINETUNING from adapted MS2 checkpoint"
echo "Checkpoint: /workspace/checkpoints/best_adapted_ms3.pt"
...
python -u offline/diffusion_policy_unet_maniskill2.py \
  ...
  --capture-video \
  --track \
  --exp-name finetune_from_ms2 \
  --load-ckpt /workspace/checkpoints/best_adapted_ms3.pt
